    ...
    const fetchData = async () => {
      try {
        const { data } = await axios.get(
          `${serverApi}mySweetBlog/withCategory/روانشناسی`
        );
        setCard(data);
      } catch (err) {
        console.log(err);
  ...
  return (
    <Box className="main-card">
      <Box className="MainCard">
        ...
        <Box width='100%' className="card-list">
                  ...
                  >
                    <Box sx={{ padding: "5px" }}>
                      <Link to={`/Document/${i.id}`}>
                        <img className="card-item-pic" width='100%' src={i.image} alt="" />
                        <Typography
                          sx={{
                            color:
