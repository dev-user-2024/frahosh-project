import { Box, Stack,Select,FormControl,MenuItem,InputLabel,IconButton  } from '@mui/material'
...
export default function MySchedule() {
    const dailySchedule=[{id:1,description:"اتمام تمرین‌های ریاضی",completed:true}]
    const weekSchedule=[{id:1,description:"رسم نمودارهای کلاس دوشنبه",completed:false}]
    const monthSchedule=[{id:1,description:"تمرین روخوانی فارسی",completed:false}]
    ...
    function SaveNewSchedule(){
        ...
        setScheduleList([...scheduleList, {id:scheduleList.length,description:newschedule, completed:false}])
        setAddSchedule(false)
    }
    function checkedSchedule(event){
        const index = scheduleList.findIndex(item=>item.id == event.target.id)
        scheduleList[index] = {...scheduleList[index], completed:event.target.checked}
  ...
  return (
    ...
    sx={{
        ...
        >
            ...
            <Button 
            buttonType="addDashboardSchedule"
        ...
        <Stack spacing={2}  mb={2} direction="column" mt={2} >
         ...
         <Stack key={index} spacing={2} alignItems="center" mb={2} direction="row" mt={2} >
            <CheckBox defaultChecked={item.completed} id={item.id} onChange={checkedSchedule} />
            <Box sx={{boxShadow:"3px 3px 30px rgba(0, 0, 0, 0.08)",width:"100%",padding:"30px 20px"}} 
