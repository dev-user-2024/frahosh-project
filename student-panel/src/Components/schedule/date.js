...
import { Grid, Typography , Box ,InputLabel  ,MenuItem ,FormControl ,Select} from "@mui/material";
...
function createDayArray(time, monthNumber, weekNumber) {
  ...
  weekNumber == 1
    ? (startDay = 1)
    : (startDay = 1 + firstWeekLength + (weekNumber - 2) * 7);
    ...
    <Fragment>
      ...
      >
        {dayList.map((item) => (
          <Box
            id={item.date}
            className={item.selected ? styles.selectedday : styles.weekday}
            onClick={selestedDayHandler}
            mr={2}
          >
            <Typography id={item.date} component="p">
              {item.day}
            </Typography>
            <Typography id={item.date} component="p">
              {item.date}
